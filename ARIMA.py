...
def train_single_ARIMA_model(column, z_train, d_alpha):
...
def trainARIMA(z: pd.DataFrame, train_window: int, d_alpha=0.05,parallel=True)->dict:
    ...
    if parallel:
        ...
        models = Parallel(n_jobs=-1)(delayed(train_single_ARIMA_model)(col, z_train, d_alpha) for col in z_train.columns)
        ...
        for c in z_train.columns:
            ...
            models[c] = pmdarima.auto_arima(z_train[c], d=d, seasonal=False, trace=True,error_action='ignore')
        return models
